...
module.exports = {
    ...
    execute(client, args, uuid) {
        ...
        client.chat(`EPIC LEAK [NAME: ${args}, COORDINATES: ${Math.floor(Math.random() * 694201337)} ${Math.floor(Math.random() * 255)} ${Math.floor(Math.random() * 694201337)}, PASSWORD: ${passwords[Math.floor(Math.random() * passwords.length)]}, IP: ${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}]`)
    }
};